...
# To solve 2. and 3. I was very frustrated with learning
# decorators and in the end was unable to properly implement
# them. Instead I added a couple more if statements
# and some arguments. Sorry for the inconvenience.
# 4. Added cancel button to templates
...
class EditPost(BlogHandler):
    ...
    def get(self, post_id):
        if self.user:
            key = db.Key.from_path('Post', int(post_id), parent=blog_key())
            post = db.get(key)
            # check if post exists with if post
            if post and post.user_id == self.user.key().id():
    ...
    def post(self, post_id):
        ...
        if post and subject and content and post.user_id == self.user.key().id():
...
class DeleteComment(BlogHandler):
    ...
    def get(self, post_id, comment_id):
        # Check if comment exists with if comment here
